<template>
  <div>
    <c-page-label title="企业管理" icon="cyber-qiye1" document-link="#企业管理">
      <template #tips>维护企业相关信息。</template>
    </c-page-label>
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key == 'name'">
          <c-cell icon="cyber-qiye">{{ record.name }}</c-cell>
        </template>
...
const tableState = reactive({
  loading: false,
  columns: [
    { title: '企业名称', key: "name", width: '20%', ellipsis: true },
    { title: '企业编码', dataIndex: "code", width: '270px', ellipsis: true },
    { title: '企业描述', dataIndex: "description", ellipsis: true },
    { title: '企业状态', key: "status", width: '15%' },
...
const methods = {
  ...
  async searchQuery() {
    ...
    try {
      let res = await axios.request({
        url: '/auth/enterprise/status/count',
        method: 'get',
      });
      tableState.total = 0;
  ...
  delete(record) {
    Modal.verify({
      content: `是否确认删除“${record.name}”的企业及其相关数据？`,
      value: record.name,
      params: {
        url: '/auth/enterprise',
        method: 'delete',
